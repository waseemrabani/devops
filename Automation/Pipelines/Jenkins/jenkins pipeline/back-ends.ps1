       ...
       stage('checkout'){
            ...
            steps{
                script{
                    ...
                    git branch: "$branch", url: "git@bitbucket.org:teamvowpay/${params.repo}.git"
                    sh "pwd"
                    sh "ls -lah"
                    }
                }
                ...
                script{
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'vowpay']]) {
                        ...
                        sh (script: "aws ecs wait services-stable --region eu-west-2 --cluster $server-vowpay --services ${params.repo}", returnStdout: true)
                            ...
                            #ALB_ARN=$(aws elbv2 describe-load-balancers --region eu-west-2 --names $ALB_Name --query 'LoadBalancers[0].LoadBalancerArn' --output text)
                            #TG_ARN=$(aws elbv2 describe-target-groups --region eu-west-2 --load-balancer-arn $ALB_ARN --query 'TargetGroups[3].TargetGroupArn' --output text)
                            #TG_ARN=$(aws elbv2 describe-target-groups  --region eu-west-2 --query "TargetGroups[?TargetGroupName=='ecs-$server-vp-$params.repo'].TargetGroupArn" --output text)
                            ...
                            curl -X POST -H 'Content-type: application/json' --data '{"text": " '"$params.repo"' service has been deployed sucessfully on branch '"$branch"'"}' https://outlook.office.com/webhook/0db64d18-467d-4b96-b8d0-81211eb86ed5@f6352f10-bd3b-410b-a5cd-c4e84e6b70a3/JenkinsCI/58f8522c80274c158f19844b67bac02f/9c7ca7dc-46e9-4d76-afdc-0e525143c834
                            break
                            fi
                            if [ $STAT = "unhealthy" ]; then
                            curl -X POST -H 'Content-type: application/json' --data '{"text": " '"$params.repo"' service deployment has been failed on branch '"$branch"' please check the application logs"}' https://outlook.office.com/webhook/0db64d18-467d-4b96-b8d0-81211eb86ed5@f6352f10-bd3b-410b-a5cd-c4e84e6b70a3/JenkinsCI/58f8522c80274c158f19844b67bac02f/9c7ca7dc-46e9-4d76-afdc-0e525143c834
                            ...
                            #curl -X POST -H 'Content-type: application/json' --data '{"text": " job '"$params.repo"'  has status  '"$STAT"'" }' https://outlook.office.com/webhook/0db64d18-467d-4b96-b8d0-81211eb86ed5@f6352f10-bd3b-410b-a5cd-c4e84e6b70a3/JenkinsCI/58f8522c80274c158f19844b67bac02f/9c7ca7dc-46e9-4d76-afdc-0e525143c834
                            '''
                        }
                    }
                }
